...
import {
  NOISE_KNOBS,
  type NoiseKnob, type NoiseModule, type NoiseSnapshot,
} from "./NoiseModule";
...
function NoiseNode({ data, id }: NodeProps<NoiseData>) {
  const { onRemove, onToggleCollapse, onUpdateParameter } = useModuleActions();
  const { speed, detail, bias, amp, collapsed } = data;

  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const snapshotRef = useRef<NoiseSnapshot | null>(null);
  const rafRef = useRef<number | null>(null);

  const [snapshot, setSnapshot] = useState<NoiseSnapshot | null>(null);
  const patched = snapshot?.patched ?? {
    speed: false, detail: false, bias: false, amp: false,
  };

  useEffect(() => {
    const mod = audioGraphManager.getModule(id) as NoiseModule | undefined;
    if (!mod) return;
    const update = (s: NoiseSnapshot) => {
  ...
  type SliderDef = { k: NoiseKnob; label: string; fmt: (v: number) => string };
  const SLIDERS: SliderDef[] = [
    { k: "speed",  label: "Speed",  fmt: (v) => v.toFixed(2) },
    { k: "detail", label: "Detail", fmt: (v) => `${1 + Math.floor(v * 4)} oct` },
    { k: "bias",   label: "Bias",   fmt: (v) => v.toFixed(2) },
    { k: "amp",    label: "Amp",    fmt: (v) => v.toFixed(2) },
  ];

  const ROW_HEIGHT = 28;
  const bottomFor = (knob: NoiseKnob): number => {
  ...
  return (
    ...
    >
          ...
          <>
            ...
            <div className="nodrag nopan">
              {SLIDERS.map((s) => {
                ...
                const dataMap: Record<NoiseKnob, number> = { speed, detail, bias, amp };
                ...
                return (
                  ...
                  >
                    <Label className={"text-[11px] font-mono font-bold w-12 shrink-0 " + (isPatched ? "text-purple-300" : "text-purple-400")}>
                      {s.label}
                    </Label>
                    <Slider
                      value={[displayVal]}
                      ...
                      aria-label={s.label}
                    />
                    <span className={"text-[10px] font-mono tabular-nums w-14 text-right " + (isPatched ? "text-purple-300 font-bold" : "text-foreground")}>
                      {s.fmt(displayVal)}
                    </span>
                  </div>
                );
      ...
      <Handle
        ...
        className="!border-2 !border-background !w-4 !h-4 !bg-purple-400"
        style={{ top: "50%" }}
      />
